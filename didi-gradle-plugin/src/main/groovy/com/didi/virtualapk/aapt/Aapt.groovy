...
public class Aapt {
    ...
    void filterPackage(final List<?> retainedTypes, final List<?> retainedStyleables, final int pp,
                       final Map<?, ?> idMaps,
                       final Map<?, ?> libRefTable, final Set<String> outUpdatedResources) {
        ...
        collectXML(assetDir, outUpdatedResources)
    }
    ...
    private static void collectXML(final File dir,final Set<String> outUpdatedResources) {
        ...
        dest.withPrintWriter { pw ->
            ...
            resources.keySet().each { type ->
                ...
                resources.get(type).each { entry ->
                    pw.println "        public static final int ${entry.resourceName} = ${entry.resourceId};"
                }
                pw.println "    }"
            }

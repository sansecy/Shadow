...
//        AppPlugin appPlugin = project.plugins.getPlugin(AppPlugin.class)
        ...
        project.afterEvaluate {
            ...
            android.applicationVariants.all { ApplicationVariant variant ->
                ...
                def installTask = project.tasks.findByName(installTaskName)
                if (installTask != null) {
                    installTask.doLast {
                        println "VAHostPlugin.apply variant = " + variant
                        println "VAHostPlugin.apply variantTaskName = " + assembleTaskName
    ...
    private void copyHostsFileToBuildDir() {
        ...
        vaHostDir.listFiles().each {
            ...
            com.google.common.io.Files.copy(it,dst)
        }
    }

    private void saveNativeLibs(ApplicationVariant variant) {
...
//            ProGuardTransform proguardTransform = proguardTask.transform
     ...
     * Save proguard mapping
     */
    def backupManifest(ApplicationVariant applicationVariant) {
        def file = new File(project.buildDir, "/intermediates/merged_manifests/${applicationVariant.name}/AndroidManifest.xml")
        project.copy {
            from file
            into vaHostDir
        }
    ...
    def generateVersion(ApplicationVariant variant) {
        FileUtil.saveFile(vaHostDir, "versions", {
            getDependList(project, variant)
        })
    }

    static List<String> getDependList(Project project, ApplicationVariant variant) {

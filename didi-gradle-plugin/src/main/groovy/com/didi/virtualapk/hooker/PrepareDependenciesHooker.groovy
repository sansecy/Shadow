...
class PrepareDependenciesHooker {
    ...
    def stripDependencies = [] as Set<DependenceInfo>
    Project project
     ...
     *
     */
    void collectVariantDependencies() {
...
//            if (hostDependencies.contains("${module[0]}:${module[1]}")) {
                def aar = new AarDependenceInfo(
                        groupId,
                        artifactId,
                        version,
                        it
                )
                if (!contain(stripDependencies, aar)) {
                ...
                if (!contain(retainedAarLibs, aar)) {
        ...
        dependenciesImpl.javaLibraries.each {
            ...
            if (hostDependencies.contains("${groupId}:${artifactId}")) {
                def jar = new JarDependenceInfo(
                        groupId,
                        artifactId,
                        version,
                        it
                )
                if (!contain(stripDependencies, jar)) {
...
//        checkDependencies()
        ...
        vaContext.putStripDependencies(virtualApk.buildingVariantKey, stripDependencies)
        println "PrepareDependenciesHooker.afterTaskExecute setStripDependencies to " + virtualApk.buildingVariantKey

        vaContext.putRetainedAarLibs(virtualApk.buildingVariantKey, retainedAarLibs)
    }

    private static boolean contain(Set<AarDependenceInfo> aarDependenceInfos, AarDependenceInfo aar) {
        aarDependenceInfos.each {
            if (it.toString().equalsIgnoreCase(aar.toString())) {
                return true
            }
        }
        return false
    ...
    private static boolean contain(Set<JarDependenceInfo> jarDependenceInfos, JarDependenceInfo jar) {
        jarDependenceInfos.each {
            if (it.toString().equalsIgnoreCase(jar.toString())) {
                return true
            }
        }
        return false

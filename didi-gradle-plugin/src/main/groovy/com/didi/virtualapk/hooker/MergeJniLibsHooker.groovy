...
class MergeJniLibsHooker {
    ...
    void beforeTaskExecute() {
        def excludeJniFiles = jniLibsCollector.collect(VAExtension.vaContext.getStripDependencies(virtualApk.buildingVariantKey))
...
//                .set('packagingOptions', new ParsedPackagingOptions(androidConfig.packagingOptions))
    ...
    Set<String> getExcludeJniFiles() {
        return jniLibsCollector.collect(VAExtension.vaContext.getStripDependencies(virtualApk.buildingVariantKey))
    }

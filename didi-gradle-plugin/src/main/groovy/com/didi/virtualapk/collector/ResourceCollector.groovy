...
class ResourceCollector {
    ...
    public ResourceCollector(Project project, ProcessAndroidResources par,ApplicationVariantImpl variant) {
...
//        reassignPluginResourceId()
        ...
        vaContext.getRetainedAarLibs(virtualApk.buildingVariantKey).each {
    ...
    private void parseResEntries(File RSymbolFile, ListMultimap resourcesMap, List styleableList) {
        println "RSymbolFile = $RSymbolFile, resourcesMap = $resourcesMap, styleableList = $styleableList"
    ...
    def getResIdMap() {
        ...
        allResources.values().each { resEntry ->
            idMap.put(resEntry.resourceId, null)
        }
        return idMap
    }
    ...
    private void dump() {
        ...
        retainTypeFile.withPrintWriter { pw ->
            pluginResources.values().each {
                pw.println "${it.resourceType} ${it.resourceName} 0x${Integer.toHexString(it.resourceId)}"
            }
        ...
        vendorTypeFile.withPrintWriter { pw ->
            vaContext.getRetainedAarLibs(virtualApk.buildingVariantKey).each { aarLib ->

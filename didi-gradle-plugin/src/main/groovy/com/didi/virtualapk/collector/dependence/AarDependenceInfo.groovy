...
//                .replace("createFullJarDebug/full.jar", "aapt")
        ...
        if (library.project != null && library.project.startsWith(":")) {
            ...
            jarFile = new File(intermediatesFile + "/runtime_library_classes_jar/" + library.projectVariant + "/classes.jar")
            assetsFolder = new File(intermediatesFile + "/library_assets/" + library.projectVariant + "/out")
            //intermediates/compressed_assets/jxSpOnlineBaseNoaiDebug/out/assets
            jniFolder = new File(intermediatesFile + "/merged_native_libs/" + library.projectVariant + "/out/lib")
        } else {

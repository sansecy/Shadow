...
class StripClassAndResTransform extends Transform {
    ...
    StripClassAndResTransform(Project project, ApplicationVariantImpl appVariant) {
        this.project = project
        this.virtualApk = project.virtualApk
        classAndResCollector = new HostClassAndResCollector(project)
        this.apkVariant = appVariant
    }
...
//        return TransformManager.CONTENT_CLASS
    ...
    boolean applyToVariant(VariantInfo variant) {
        return virtualApk.stripResource
    }
...
/**
    ...
    void transform(final TransformInvocation transformInvocation) throws TransformException, InterruptedException, IOException {
        ...
        if (!virtualApk.stripResource) {
            println "StripClassAndResTransform.transform not stripResource"
            return
        }
//        vaContext.getStripDependencies("${project.name}:${apkVariant.name}").each {
////            println "[StripClassAndResTransform.transform] stripDependencie = ${it.group} :${it.artifact}: ${it.version}"
//        }
        LinkedHashSet stripEntries = classAndResCollector.collect(vaContext.getStripDependencies(virtualApk.buildingVariantKey))

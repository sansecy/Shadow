...
class VAExtension {
    ...
    public String buildingVariantKey
...
//                println "VAExtention.getHostDependencies key = "+"${module.group}:${module.name}"+",value=${module}"
    ...
    public static class VAContext {
        ...
        public HashMap<String, Collection<DependenceInfo>> stripDependenciesMap = []
        public HashMap<String, Collection<AarDependenceInfo>> retainedAarLibsMap = []

        Collection<DependenceInfo> getStripDependencies(String key) {
            return stripDependenciesMap.get(key)
        }

        Collection<DependenceInfo> peekFirstStripDependencies() {
            return stripDependenciesMap.get(stripDependenciesMap.keySet().iterator().next())
        }

        void putStripDependencies(String key, Collection<DependenceInfo> dependenceInfoCollection) {
            stripDependenciesMap.put(key, dependenceInfoCollection)
        }

        Collection<AarDependenceInfo> getRetainedAarLibs(String key) {
            return retainedAarLibsMap.get(key)
        }

        Collection<AarDependenceInfo> peekFirstRetainedAarLibs() {
            return retainedAarLibsMap.get(retainedAarLibsMap.keySet().iterator().next())
        }

        void putRetainedAarLibs(String key, Collection<AarDependenceInfo> dependenceInfoCollection) {
            retainedAarLibsMap.put(key, dependenceInfoCollection)
        }

...
//        def key = pluginExt.packageId
//        def libRefTable = new HashMap<>()
//        libRefTable.put(0x6E, par.applicationId.get())
        def libRefTable = ["${pluginExt.packageId}": pluginExt.manifestPackage]
...
//        }
        //追加完资源打包一份只包含资源的apk查看
        project.copy {
            from apFile
            into apFile.getParentFile()
            rename { "${Files.getNameWithoutExtension(apFile.name)}-new.apk" }
        }
...
//
//        Set<String> hostClasses = new HostClassAndResCollector(project).collect(VAExtension.vaContext.getStripDependencies(virtualApk.buildingVariantKey))
...
//        updateRJava(aapt, RClassOutputJar, originalApplicationId, resourceCollector)
     ...
     * $ aapt add resources.ap_ file1 file2
     */
    def addUpdatedResources(String aaptPath, List<String> param) {
        project.exec {
            executable aaptPath
            args param
            // store the output instead of printing to the console
    ...
    def static convertResourcesForAapt(ListMultimap<String, ResourceEntry> pluginResources) {
        ...
        pluginResources.keySet().each { resType ->
            ...
            def typeEntry = [type   : "int", name: resType,
                             ...
                             _id    : parseTypeIdFromResId(firstEntry.resourceId),
                             entries: []]
            ...
            pluginResources.get(resType).each { resEntry ->
                typeEntry.entries.add([
                        ...
                        _id : parseEntryIdFromResId(resEntry.resourceId),
                        v   : resEntry.resourceId, _v: resEntry.resourceId,
                        vs  : resEntry.hexResourceId, _vs: resEntry.hexResourceId])
            }
    ...
    def updateRJava(Aapt aapt, File RClassOutputJar, String originalApplicationId,
        ...
        println "删除原始R.java，重新生成新的R.java文本"
...
//        VAExtension.getVaContext().splitRJavaFile = splitRSourceFile
        ...
        resourceCollector.vaContext.getRetainedAarLibs(virtualApk.buildingVariantKey).each {
            def aarPackage = it.package
            println "aarPackage = $aarPackage"
